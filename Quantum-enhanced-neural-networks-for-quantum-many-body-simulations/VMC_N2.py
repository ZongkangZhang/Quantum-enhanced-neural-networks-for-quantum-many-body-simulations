...
def main():
    ...
    if rank == 0:
        ...
        if opt_method == 'Adam':
            ...
            reg_coef_init = 0.08  # 0.0 0.08  # regularizer
            reg_stage = 1000  # 0 800 1000
           
            lr_step_size = 200 
            lr_boundaries = list(range(lr_step_size, epoch, lr_step_size))  
            lr_decay_rate = 0.9  # 0.92  
            lr_values = [lr_init * (lr_decay_rate ** i) for i in range(len(lr_boundaries) + 1)] 
    ...
    ansatz_reps = 20  
    entanglement = 'circular'  # 'full' 'circular' 'pairwise' 'linear'
    ansatz = create_ansatz(Nq, entanglement, ansatz_reps, simulator, shots, diff_method)
    std_ansatz = 0.01  # 0.01 0.5
    std_coef = 0.01  # 0.01 0.5
    ...
    for t in range(epoch):    
        if rank == 0:
            ...
            lr =  step_decay(t, lr_boundaries, lr_values)  # step
